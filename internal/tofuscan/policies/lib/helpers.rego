...
# the given metric name via the logging.googleapis.com/user/ prefix. Used by
# CIS 2.5–2.12 policies to verify that an alerting policy exists for each
# required log metric.
alert_exists_for_metric(metric_name) if {
	...
	contains(object.get(threshold, "filter", ""), concat("", ["logging.googleapis.com/user/", metric_name]))
}
