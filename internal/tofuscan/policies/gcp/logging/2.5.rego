...
# True when at least one metric matching the ownership filter is wired to an alert.
_has_ownership_metric_with_alert if {
...
deny contains _violation_2_5 if {
...
deny contains _violation_2_5 if {
	input.resource.google_project
	lib.metric_filter_exists("roles/owner")
	not _has_ownership_metric_with_alert
}
