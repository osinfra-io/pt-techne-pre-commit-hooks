...
| 1.6 | | Ensure That IAM Users Are Not Assigned the Service Account User or Service Account Token Creator Roles at Project Level | Automated | | Assign the `serviceAccountUser` and `serviceAccountTokenCreator` roles at the service account level, not the project level, to limit blast radius. |
