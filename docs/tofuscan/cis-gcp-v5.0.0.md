...
Google Cloud Platform. Controls focus on securing super admin accounts, enforcing
organization-level constraints, managing service account permissions, rotating
cryptographic keys, and protecting API credentials.

### 1.1 Super Admin Account Management

| Control | Level | Title | Type | Covered | Description |
| --------- | ------- | ------- | ------ | --------- | ------------- |
| 1.1.1 | Level 1 | Ensure Super Admin Email Address Is Not Tied To A Single User | Manual | | The super admin email should be a shared inbox or group address, not a personal account, to ensure continuity and prevent a single point of administrative failure. |
| 1.1.2 | Level 1 | Ensure Super Admin Account Is Not Used For Google Cloud Administration | Manual | | Super admin accounts should be reserved for identity management only; day-to-day GCP administration should use separate, dedicated admin accounts to limit exposure. |
| 1.1.3 | Level 2 | Ensure Folders Are Structured By Environment And Sensitivity | Manual | | Organize the GCP resource hierarchy with folders that reflect environment (production, non-production) and data sensitivity to enable consistent policy and access control boundaries. |
| 1.1.4 | Level 2 | Ensure Organization Policies Are Configured For Centralized Constraints | Manual | | Apply organization policies at the organization or folder level to centrally enforce security constraints — such as restricting resource locations and disabling service account key creation — across all projects. |
...
| 3.8 | Level 2 | Ensure VPC Service Controls Is Enabled for Supported Google Cloud Services | Manual | | VPC Service Controls create a security perimeter around GCP API services to mitigate data exfiltration risks and restrict API access to approved networks and identities. |
| 3.9 | Level 2 | Ensure Private Service Connect is Used for Access to Google APIs | Manual | | Private Service Connect enables private connectivity to Google APIs through internal IP addresses within a VPC, preventing API traffic from traversing the public internet. |
| 3.10 | Level 2 | Ensure that VPC Flow Logs is Enabled for Every Subnet in a VPC Network | Automated | ✅ | VPC Flow Logs capture metadata about IP traffic on network interfaces, enabling network forensics, anomaly detection, and security auditing. |
| 3.11 | Level 1 | Ensure No HTTPS or SSL Proxy Load Balancers Permit SSL Policies With Weak Cipher Suites | Manual | ✅ | SSL policies control which TLS versions and cipher suites clients may use. Weak cipher suites should be explicitly excluded to prevent downgrade attacks. This control is classifiable via IaC inspection. |
| 3.12 | Level 1 | Use Identity Aware Proxy (IAP) to Ensure Only Traffic From Google IP Addresses are Allowed | Manual | | IAP authenticates user requests via Google SSO. Combine IAP with firewall rules that only allow traffic from Google's IP ranges for defence-in-depth. |
...
| 6.2.2 | Level 1 | Ensure That the Log_connections Database Flag for Cloud SQL PostgreSQL Instance Is Set to On | Automated | ✅ | Enabling `log_connections` logs each successful connection to the database server, supporting audit trails for access monitoring. |
| 6.2.3 | Level 1 | Ensure That the Log_disconnections Database Flag for Cloud SQL PostgreSQL Instance Is Set to On | Automated | ✅ | Enabling `log_disconnections` logs session termination details, providing a complete picture of database session activity alongside `log_connections`. |
| 6.2.4 | Level 1 | Ensure Log_statement Database Flag for Cloud SQL PostgreSQL Instance Is Set Appropriately | Automated | ✅ | The `log_statement` flag controls which SQL statements are logged. It should be set to `ddl` or stricter to capture schema-level changes. |
| 6.2.5 | Level 1 | Ensure that the Log_min_messages Flag for a Cloud SQL PostgreSQL Instance Is Set to Warning or Stricter | Automated | ✅ | Setting `log_min_messages` to `WARNING` or stricter ensures that only meaningful diagnostic messages are logged, balancing observability with log volume. |
| 6.2.6 | Level 1 | Ensure Log_min_error_statement Database Flag for Cloud SQL PostgreSQL Instance Is Set to Error or Stricter | Automated | ✅ | Controls the minimum severity level for logging the SQL statement that caused a logged error, helping correlate errors with the queries that triggered them. |
| 6.2.7 | Level 1 | Ensure That the Log_min_duration_statement Database Flag for Cloud SQL PostgreSQL Instance Is Set to -1 | Automated | ✅ | Setting `log_min_duration_statement=-1` disables logging of statement durations, preventing unintentional inclusion of query data (including literals) in logs. |
...
| 6.6 | Level 1 | Ensure Cloud SQL Database Instances Have IAM Database Authentication Enabled | Manual | | IAM database authentication allows users and service accounts to authenticate to Cloud SQL using IAM credentials instead of database passwords, centralizing access control and eliminating shared credentials. |
| 6.7 | Level 2 | Ensure That Cloud SQL Database Instances Do Not Have Public IPs | Automated | ✅ | Cloud SQL instances should use private IP addresses only, accessible within a VPC network, to avoid direct internet exposure. |
| 6.8 | Level 1 | Ensure That Cloud SQL Database Instances Are Configured With Automated Backups | Automated | ✅ | Automated backups ensure that a recovery point is available in the event of data loss, corruption, or accidental deletion. |
| 6.9 | Level 1 | Ensure Cloud SQL Database Instances Have Deletion Protection Enabled | Manual | | Enabling deletion protection on Cloud SQL instances prevents accidental or unauthorized deletion of production databases, reducing risk of unrecoverable data loss. |
